...
class Chat {
    ...
    constructor($status?: any) {
        this.startConversation(secret).then((res: {token: string, conversationId: string, streamUrl: string}) => {
            token = res.token;
            this.$status = $status;
            console.log('res: ', res);
    ...
    getMessage(watermark_value = 0) {
        return new Promise((resolve, reject) => {
            const request = new XMLHttpRequest();
            this.reconnect(watermark_value).then((res: {token:string}) => {
                token = res.token;
    ...
    public sendMessage(message = {}) {
        return new Promise((resolve, reject) => {
            ...
            message = (<any>Object).assign(appendix, message);

            this.$status.append('<p>' + 'token expired! try reconnect... ');
            this.reconnect().then((res: {token: string}) => {
                token = res.token;
